...
class LlamaMLP(nn.Module):
    def __init__(self, config):
        ...
        self.activation_mask = nn.Parameter(torch.zeros(config.hidden_size, requires_grad=False), requires_grad=False)
        self.applied_module = 'None'

    def forward(self, x):
        down_proj = self.down_proj(self.act_fn(self.gate_proj(x)) * self.up_proj(x))
        if self.applied_module == 'ffn':
            # print('current processing ffn steering:')
            # print('self.activation_mask:',self.activation_mask.shape,'down_proj:',down_proj.shape)
            down_proj[:, :, :] = down_proj[:, :, :] + down_proj[:, :, :] * self.activation_mask
        return down_proj
    ...
    def forward(
        ...
        **kwargs: Unpack[FlashAttentionKwargs],
        ...
        if self.applied_module == 'attention':
            attn_output[:, :, :] = attn_output[:, :, :] + attn_output[:, :, :] * self.activation_mask
            # attn_output[:, :, :] = attn_output[:, :, :] + self.activation_mask
...
class LlamaDecoderLayer(nn.Module):
    ...
    def forward(
        ...
        **kwargs: Unpack[FlashAttentionKwargs],
        ...
        if self.applied_module == 'layer':
            # print('current processing layer steering:')
            # print('self.activation_mask:', self.activation_mask.shape, 'attn_output:', hidden_states.shape)
            hidden_states[:, :, :] = hidden_states[:, :, :] + hidden_states[:, :, :] * self.activation_mask
...
class LlamaForCausalLM(LlamaPreTrainedModel, GenerationMixin):
    ...
    #     # model = model.to(torch.device(device), dtype=torch_dtype)
    #     return model
    ...
    def custom_from_pretrained(
        ...
        # ❌ DO NOT: model.to("cuda") for 4-bit models
        return model
