...
class Qwen3Attention(nn.Module):
    ...
    def __init__(self, config: Qwen3Config, layer_idx: int):
        ...
        ## revised code for our method:
        # self.attn_A = nn.ParameterList(
        #     [nn.Parameter(torch.zeros(self.head_dim, requires_grad=True), requires_grad=True) for _ in
        ...
        #     [nn.Parameter(torch.zeros(self.head_dim, requires_grad=True), requires_grad=True) for _ in
        #      range(self.num_heads)])
        # # print('attn_v:',len(self.attn_v),self.attn_v[0].shape)
        # self.attn_b = nn.ParameterList(
        #     [nn.Parameter(torch.zeros(self.head_dim, requires_grad=True), requires_grad=True) for _ in
        #      range(self.num_heads)])
        # # print('attn_b:', len(self.attn_b), self.attn_b[0].shape)
        # # self.main_comp =  nn.ParameterList([nn.Parameter(torch.zeros(self.head_dim,requires_grad=False),requires_grad=False) for _ in range(self.num_heads)])
        # self.main_comp = nn.ParameterList(
        #     [nn.Parameter(torch.zeros(self.head_dim, self.head_dim), requires_grad=False) for _ in
        #      range(self.num_heads)])
        ### If appplied_module == 'attention,' LoFiT is applied to the attention output
        self.activation_mask = nn.Parameter(torch.zeros(config.head_dim * config.num_attention_heads, requires_grad=False), requires_grad=False)
        self.applied_module = 'None'
